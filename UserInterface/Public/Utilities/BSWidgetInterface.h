...
public:
	...
	/** Returns the enum value corresponding to the string, or if not found, the enum byte value of 0 for T. */
	template <typename T>
	...
	/** Returns a pointer of type T, recursively searching only until it finds the first instances of T inside the
	 *  PanelWidget. */
	template <typename T>
