...
/** The base button used for BeatShot user interface. \n \n \b- Supports mutually exclusive buttons being "active" at
 *  once using pointers to other buttons and the SetActive and SetInActive functions, but can also be used for buttons
 *  not mutually exclusive if RestoreClickedButtonState is set to true. \n \b-  \n \b- Create functions in child classes
 *  like the SetDefaults function with additional parameters that can be retrieved using the OnBSButtonPressed delegate.
 */
UCLASS()
...
public:
	...
	/** Called when another linked BSButton is clicked, reverses the OnPressed animation to restore it to pre-animated
	 *  state. */
	...
	/** Returns whether this button has had its defaults set (next pointer and other data). */
...
protected:
	...
	/** Image shown as the button; Expected to be a material instance with parameters that change the material when
	 * pressed or hovered. */
	...
	/** Whether to restore the pre-pressed state of the button after pressing. This just sets the PressedScalarParameter
	 *  value to 0 after the animation finishes. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "BSButton|Defaults", meta = (ExposeOnSpawn="true"))
